...
| 变量名                    | 值                            | 说明                                   |
| ------------------------- | ----------------------------- | -------------------------------------- |
| `NODE_VERSION`            | `22`                          | 与 `.nvmrc` 和 `package.json` 保持一致 |
...
> 本 Fork 默认不提交 `wrangler.toml`，由 Cloudflare dashboard 管理环境变量，避免误用模板演示站的域名和评论配置。如果以后新增 `wrangler.toml`，请注意它会接管 Pages 环境变量；届时必须把 `SITE_URL` 和所有 `PUBLIC_*` 变量同步写入该文件。
...
- 去 Cloudflare Pages → **Settings** → **Environment variables**，把 `SITE_URL` 改成 `https://anvilquestwiki.wiki`。
